...
```
Untuk mengetahui hubungan antar variabel maka dibuatlah scatterplot agar lebh mudah untuk dilihat.
```{r}
...
```{r}
library(GGally)
ggpairs(marketing,lower = list(continuous = wrap('points', colour = "yellow")),
...
```{r}
model_mlr <- lm(sales ~ ., data = marketing)
summary(model_mlr)
```
